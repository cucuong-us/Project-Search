...
# Node class đại diện cho trạng thái của Ares và các viên đá
class Node:
    def __init__(self, ares_x, ares_y, stones, grid, parent=None, action=None, cost=0):
        self.ares_x = ares_x
        self.ares_y = ares_y
        self.stones = stones
        self.grid = grid
    ...
    def __lt__(self, other):
        return self.total_cost < other.total_cost  # So sánh dựa trên tổng chi phí

    def is_goal(self):
        # Kiểm tra xem tất cả đá đã ở vị trí công tắc chưa
    ...
    def get_neighbors(self, stone_weights):
        ...
        for dx, dy, move, push in directions:
            ...
            # Đẩy đá
                ...
                if self.grid[behind_stone_x][behind_stone_y] in [' ', '.']: #chỉnh chỗ này
                    new_node = self.clone()
    ...
    def clone(self):
        return Node(self.ares_x, self.ares_y, self.stones.copy(), [row.copy() for row in self.grid], self, self.action, self.cost)
...
def ucs_search(start_node, stone_weights):
    frontier = []
    heapq.heappush(frontier, (0, start_node))
    explored = {}  # Chuyển từ set sang dict để lưu chi phí của trạng thái
    node_count = 0
    tracemalloc.start()
    ...
    while frontier:
        ...
        if state not in explored or current_cost < explored[state]:
            explored[state] = current_cost  # Cập nhật chi phí mới thấp hơn
        ...
        for i, line in enumerate(file):
            ...
            for j, char in enumerate(line.rstrip()):
                if char == '@' or char == '+':
                    ...
                    stones.append((i, j))
        return stone_weights, grid, ares_x, ares_y, stones
...
def solve_ucs(input_file, output_file):
    # Đọc input từ file
    stone_weights, grid, ares_x, ares_y, stones = read_maze(input_file)
    # Khởi tạo node bắt đầu
    start_node = Node(ares_x, ares_y, stones, grid)
    # Thực hiện UCS
    goal_node, node_count, time_taken, memory_used = ucs_search(start_node, stone_weights)
    if goal_node:
        ...
        # Ghi output
        write_output(output_file, 'UCS', steps, total_weight, node_count, time_taken, memory_used, solution)
        print(f"Solution written to {output_file}")
    else:
        print("No solution")
        write_output(output_file, 'UCS', 0, 0, node_count, time_taken, memory_used, "No solution")
